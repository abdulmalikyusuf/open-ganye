...
export default function ProjectsProvider() {
    ...
    React.useEffect(()=> {
        ...
        {
            const ganye = data.data.filter(project => project.project_lga.lga_name === "Ganye")
            setProjects(parseProjectData(ganye))
            setRawProjectData(ganye)
        }, (err) => err.response)
        // const ganye = projectsData.data.filter(project => project.project_lga.lga_name === "Ganye")
        // setProjects(parseProjectData(ganye))
        // setRawProjectData(ganye)
    }, [])
